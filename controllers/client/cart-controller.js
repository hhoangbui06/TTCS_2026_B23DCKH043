...
module.exports.postAddProduct = async (req, res) => {
  ...
  if (cart) {
    ...
    if (!checkExistsProduct) {
      ...
      await dataProducts.updateOne({ _id: productId }, { $inc: { stock: -(quantity) } })
...
module.exports.deleteProduct = async (req, res) => {
  ...
  let cartId = req.cookies.cartId
  let product = await dataCart.findOne(
    { 'products.product_id': id , _id:cartId},
    { 'products.$': 1 }
  )
  let deletedQuantity = product.products[0].quantity;
  await dataCart.updateOne({ _id: cartId }, { $pull: { products: { product_id: id } } })
  await dataProducts.updateOne({_id:id}, {$inc:{stock:deletedQuantity}})
